...
func set_TileData_on(TSName:String, posV3:Vector3, tileData:TileData) -> bool:
    return _CurrentSav.set_tile_on(TSName, posV3, tileData)
...
func get_TileData_on(TSName:String, posV3:Vector3) -> TileData:
    var savResult := _CurrentSav.get_tile_on(TSName, posV3)
    if(savResult.tileID == -1): return _CurrentMap.get_tile_on(TSName, posV3)
    return savResult
